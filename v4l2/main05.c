...
static void v4l2_read_data(void)
{
    ...
    // for ( ; ; ) 
    // {
        long frm_size = frm_width * frm_height;
        for(buf.index = 0; buf.index < FRAMEBUFFER_COUNT; buf.index++) 
        {
            ...
            printf("\r\n\r\n");
            unsigned short *ptr = buf_infos[buf.index].start;
            for (int i = 0; i < buf_infos[buf.index].length; i++)
            {
                printf("%#x ",ptr[i]);
            }
            printf("\r\n");

            printf("\r\nwidth:%d height:%d  size: %ld\r\n",frm_width,frm_height,frm_size);
    ...
    // }
    //关闭视频采集
    if (0 > ioctl(v4l2_fd, VIDIOC_STREAMOFF, &buf.type)) 
    {
