...
static int file_fd = -1;                //摄像头设备文件描述符
...
static void v4l2_read_data(void)
{
    ...
    // {
        ...
        ret = write(file_fd, buf_infos[buf.index].start ,buf_infos[buf.index].length);
        if (ret == -1)
        {
...
int process_frame()
{
    ...
    fd_set fdread;
    FD_ZERO(&fdread);
    FD_SET(v4l2_fd, &fdread);
    ret = select(v4l2_fd + 1, &fdread, NULL, NULL, &tvptr);
    if (ret == -1){
        perror("EXIT_FAILURE");
        exit(EXIT_FAILURE);
    }
